...
// 用 ref 追踪易变状态，避免 fetchHistory 频繁重建导致 effect 循环
  ...
  const handleGenerateReport = async (mode: string) => {
    ...
    try {
      ...
      setStoreError(getParsedApiError(err instanceof Error ? err.message : '批量分析启动失败'));
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 w-full min-w-0 flex-1" style={{ maxWidth: 'min(100%, 1168px)' }}>
          ...
          <button
            ...
            className="btn-purple flex items-center gap-1.5 whitespace-nowrap flex-shrink-0"
          >
            {isGeneratingReport ? (
              <>
                <svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24">
      ...
      {showBatchConfirm && (
        ...
        >
          <div
            className="glass-card p-6 max-w-sm mx-4 shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-white font-medium text-lg">生成今日报告</h3>
            </div>
            
            <p className="text-sm text-secondary mb-5">
              选择要生成的分析报告类型（无视交易日检查）：
            </p>
            ...
            <div className="space-y-3 mb-6">
              <button
                ...
                disabled={isGeneratingReport}
                className="w-full flex items-center justify-between px-4 py-3 rounded-lg bg-cyan/10 border border-cyan/20 text-cyan hover:bg-cyan/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed group"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-cyan/20 flex items-center justify-center group-hover:bg-cyan/30">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                    </svg>
                  </div>
                  <div className="text-left">
                    <div className="font-medium text-white">生成自选个股报告</div>
                    <div className="text-xs text-secondary">仅分析自选股列表</div>
                  </div>
                </div>
                <svg className="w-4 h-4 text-cyan/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                disabled={isGeneratingReport}
                className="w-full flex items-center justify-between px-4 py-3 rounded-lg bg-purple/10 border border-purple/20 text-purple hover:bg-purple/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed group"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-purple/20 flex items-center justify-center group-hover:bg-purple/30">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                    </svg>
                  </div>
                  <div className="text-left">
                    <div className="font-medium text-white">生成大盘报告</div>
                    <div className="text-xs text-secondary">仅执行大盘复盘分析</div>
                  </div>
                </div>
              ...
              <button
                ...
                disabled={isGeneratingReport}
                className="w-full flex items-center justify-between px-4 py-3 rounded-lg bg-gradient-to-r from-cyan/10 to-purple/10 border border-white/10 text-white hover:from-cyan/20 hover:to-purple/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed group"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-r from-cyan/20 to-purple/20 flex items-center justify-center group-hover:from-cyan/30 group-hover:to-purple/30">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                    </svg>
                  </div>
                  <div className="text-left">
                    <div className="font-medium text-white">生成全部报告</div>
                    <div className="text-xs text-secondary">个股分析 + 大盘复盘</div>
                  </div>
                </div>
            ...
            <button
              onClick={() => setShowBatchConfirm(false)}
              className="w-full px-4 py-2.5 rounded-lg text-sm text-secondary hover:text-white hover:bg-white/5 border border-white/10 transition-colors"
            >
              取消
            </button>
          </div>
