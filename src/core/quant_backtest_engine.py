...
class QuantBacktestEngine:
    ...
    def _process_exits(
        ...
        for code, position in list(open_positions.items()):
            ...
            if not position.tp1_done and bar.high >= position.entry_price + 2 * position.r_value:
                sell_shares = self._calc_partial_shares(position.shares, fraction=1 / 3)
                if sell_shares > 0:
                    exit_price = position.entry_price + 2 * position.r_value
                    trades.append(self._close_slice(position, current_date, exit_price, sell_shares, "take_profit_2r"))
            ...
            if not position.tp2_done and bar.high >= position.entry_price + 4 * position.r_value:
                sell_shares = self._calc_partial_shares(position.shares, fraction=1 / 2)
                if sell_shares > 0:
                    exit_price = position.entry_price + 4 * position.r_value
                    trades.append(self._close_slice(position, current_date, exit_price, sell_shares, "take_profit_4r"))
                    cash += self._calc_exit_cash(exit_price, sell_shares)
                    position.shares -= sell_shares
                    position.tp2_done = True
                    position.current_stop_price = max(position.current_stop_price, position.entry_price + position.r_value)

            if position.shares <= 0:
                del open_positions[code]
                continue

            if position.max_high_since_entry >= position.entry_price + 3 * position.r_value:
                trailing_stop = position.max_high_since_entry - 1.5 * position.r_value
                position.current_stop_price = max(position.current_stop_price, trailing_stop)
    ...
    def _should_time_stop(*, position: OpenPosition, bar: DailyBar, holding_days: int) -> bool:
        """Trigger stale-trade failure exit only after both age and weakness are confirmed."""
        if holding_days <= 0:
            return False

        # Keep the stale-trade exit deterministic and close to the original baseline behavior.
        if position.entry_module == "BREAKOUT":
            return (
                holding_days >= 5
                and position.max_high_since_entry < position.entry_price + position.r_value
            )
        if position.entry_module == "PULLBACK":
            return (
                holding_days >= 8
                and position.max_high_since_entry < position.entry_price + 1.5 * position.r_value
            )
        if position.entry_module == "LATE_WEAK_TO_STRONG":
            return (
                holding_days >= 5
                and position.max_high_since_entry < position.entry_price + position.r_value
            )
