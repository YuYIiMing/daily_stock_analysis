...
#### 9.1 趋势推论（优先级最高）
- **触发条件**：股价处于标准多头排列（MA5 > MA10 > MA20）且创近期新高
- **结论**：自动判定"获利盘占比高，筹码结构健康，无套牢盘压力"

#### 9.2 量能推论（次优先级）
- **触发条件**：股价横盘但换手率极低(< 2%)且量比萎缩
- **结论**：判定为"筹码高度锁定，主力锁仓迹象明显"

#### 9.3 风险推论（最低优先级）
- **触发条件**：股价处于空头排列且远离 MA20
- **结论**：判定为"上方堆积大量套牢盘，筹码极其散乱"

#### 9.4 替代描述格式
- 在检查清单中将"筹码数据缺失"改为"基于量价推导：[健康/松动/散乱]"
