...
| 量化 | 概念板块趋势系统 | 独立 `quant_*` 链路，按同花顺概念板块口径拆分市场状态、板块阶段、四模块信号、组合回测与次日交易清单 |
...
- Web UI 新增独立的「量化策略」工作台，不与原有 AI 回测页面混用
- 固定使用同花顺概念板块作为主拆分口径，行业板块仅作辅助展示
- 当前提供 `突破 / 回调 / 后期强趋势回踩 / 后期弱转强` 四个日频模块，按 `T 日收盘生成信号，T+1 开盘执行`
- 日线量化趋势系统 V1 现已对齐为“初期试错 / 中期主做 / 后期强趋势谨慎参与”的阶段语义；退出逻辑改为 `10% / 20%` 分批止盈 + 趋势/情绪退出，弱化原先偏工程化的 `2R / 4R + 强 time stop` 口径
- `BREAKOUT` 候选打分现已加入突破质量权重，会根据突破幅度、放量强度、收盘位置、平台紧凑度、相对 `ma5` 偏离和近期重复突破次数做小幅加减分，让强确认突破更容易排前、边缘突破自然降权
- 同步状态卡现支持页面内手动执行 `仅同步最新日` 与 `全窗口重同步`；同步完成后只刷新同步状态与交易计划，不会自动触发回测
- 页面内的“仅同步最新日”现走轻量同步路径，只补最近窗口所需的个股日线、当日概念归属和当日板块快照，减少被浏览器请求超时误判为失败的概率
- 若当日 `stock_daily / index / concept membership / board` 原始数据已在库中，“仅同步最新日”会直接跳过外部抓取，仅重建最新特征，降低再次触发外部超时的概率
- `GET /api/v1/quant-strategy/sync-status` 可直接查看概念板块覆盖数、概念归属覆盖股票数、主板股票池规模、已落库日线股票数、最新个股特征日期等同步状态
- 同步状态卡会直接提示“当日数据已完成 / 全量历史补齐中 / 全量历史已补齐”，减少手动比对日期和覆盖数的成本
- 量化同步默认股票池已切换为“全市场主板股票清单”，不再只依赖 `stock_daily` 里的已有样本股
- 特征重建已支持按股票分批执行，并可通过 `scripts/sync_quant_strategy.py --feature-batch-size ... --latest-feature-only` 只重建最新信号日，避免大股票池下 OOM
- Linux 容器运行时会优先使用 `pysqlite3-binary` 提供的新版 SQLite，降低宿主机/容器 SQLite 版本差异导致的数据库兼容问题
- 次日交易候选生成在板块代码不一致或同日板块特征临时缺失时，会优先按板块名称匹配并回退到最近可用板块特征，降低“假空计划”的风险
- 概念归属快照现优先按同花顺概念详情页的成分股拆分生成，板块目录与板块成分统一使用 THS 名称口径，减少 `绿色电力` 这类“板块有快照但成分归属不同名”的错配
- 板块特征构建在 `board_code` 对不上但板块名一致或可安全别名映射时，会继承同日成分股广度、涨停数和龙头信息，避免真实强板块被误写成 `strong_stock_count=0 / limit_up_count=0`
- 候选与空计划诊断已支持常见概念板块别名匹配，例如 `新能源 -> 新能源汽车`、`CPO概念 -> 共封装光学(CPO)`、`电网概念 -> 智能电网`，减少因命名差异造成的板块特征假缺口
- 无有效概念归属的股票会在特征构建阶段被标记为 `eligible_universe=false`，不再混入“概念板块趋势系统”的可交易股票池；历史残留的 `未归类概念` 占位板块也会在候选生成时被忽略
- 当交易计划为空时，接口与页面会返回空计划诊断，显示合格股票池数量、板块特征缺口、允许交易板块数量、阶段就绪股票数和主要阻断原因，便于区分“策略空仓”和“数据异常”
- 空计划诊断现额外显示“阶段就绪分布”和“形态阻断分布”，可直接区分“后期入场未触发”与“初期/中期个股形态尚未触发”
- Web UI 的空计划区域已改成“结论 → 三层筛选路径 → 三类卡点 → 细节展开”的阅读顺序，优先回答“今天为什么没有计划单”，避免把诊断数字误看成交易信号
- 结构化回测汇总口径已与落库成交账本统一：`summary.trade_count / win_count / loss_count / win_rate_pct` 与 `quant_trade_ledger` 使用同一套“合并后成交”计数基础，避免页面摘要与逐笔交易条数不一致
- 已执行交易明细现额外返回并展示股票名称、买入金额和卖出金额；若本地暂无可靠名称，会稳定回退显示为 `股票+代码`，避免列表出现空白名称
- 股票名称解析现优先读取本地 `stock_directory` 主数据表；该表在量化同步时批量更新，不再依赖交易接口临时访问第三方数据源补名
- 后端新增结构化接口：
  - `POST /api/v1/quant-strategy/sync`
  - `GET /api/v1/quant-strategy/sync-status`
  - `POST /api/v1/quant-strategy/backtests/run`
  - `GET /api/v1/quant-strategy/backtests/{run_id}`
  - `GET /api/v1/quant-strategy/backtests/{run_id}/trades`
  - `GET /api/v1/quant-strategy/backtests/{run_id}/equity`
  - `GET /api/v1/quant-strategy/trade-plan`
...
在 **首页** 点击「生成今日报告」按钮（紫色，位于分析按钮右侧）可触发自选股批量分析：

- **完整流程**：自动执行所有自选股的个股分析、大盘复盘（如启用）和通知发送
- **二次确认**：点击后弹出确认对话框，防止误操作
- **防重复提交**：已有批量任务进行时会提示等待，避免重复触发
- **异步执行**：任务在后台执行，不影响 Web UI 操作
- **进度提示**：按钮在任务执行期间置灰并显示加载状态

**API**：`POST /api/v1/analysis/batch`，返回 `task_id` 和待分析股票数量。详见 [完整指南](docs/full-guide.md)。
