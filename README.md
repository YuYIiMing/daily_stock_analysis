...
| 量化 | 概念板块趋势系统 | 独立 `quant_*` 链路，按同花顺概念板块口径拆分市场状态、板块阶段、三模块信号、组合回测与次日交易清单 |
...
- 当前提供 `突破 / 回调 / 后期弱转强` 三个日频模块，按 `T 日收盘生成信号，T+1 开盘执行`
...
- 空计划诊断现额外显示“阶段就绪分布”和“形态阻断分布”，可直接区分“后期默认不做”与“初期/中期个股形态尚未触发”
