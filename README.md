...
| **趋势系统** | **全市场趋势辅助** | **独立于自选股的 A 股趋势交易辅助系统：指数定仓、主线板块扫描、龙头/龙二识别、候选股与买点、持仓管理、纪律状态机、明日计划** |
...
> 趋势系统页面默认采用“只读快照”模式：访问 `/trend-system` 不会触发全量重算；手动点击「重算」会提交后台任务并立即返回，不再阻塞页面，并带有单飞锁（single-flight）避免并发重复重建。
...
| `TREND_SYSTEM_ENABLED` | 启用 A 股趋势交易系统页面与 API（默认 `true`） | 可选 |
| `TREND_SYSTEM_DEGRADED_MIN_FLOAT_MARKET_CAP_B` | 趋势系统进入失效观察模式后使用的最小流通市值阈值（单位：亿，默认 `100`） | 可选 |
| `TREND_SYSTEM_DEGRADED_MIN_AVG_AMOUNT_B` | 趋势系统进入失效观察模式后使用的最小日均成交额阈值（单位：亿，默认 `5`） | 可选 |
